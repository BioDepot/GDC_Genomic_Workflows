...
class OWFile(OWBwBWidget):
    ...
    inputs = [("File",str,"handleInputsFile"),("File2",str,"handleInputsFile2"),("File3",str,"handleInputsFile3")]
    outputs = [("File",str)]
    ...
    def handleInputsFile2(self, value, *args):
        if args and len(args) > 0: 
            self.handleInputs("File2", value, args[0][0], test=args[0][3])
        else:
            self.handleInputs("inputFile", value, None, False)
    def handleInputsFile3(self, value, *args):
        if args and len(args) > 0: 
            self.handleInputs("File3", value, args[0][0], test=args[0][3])
        else:
            self.handleInputs("inputFile", value, None, False)
